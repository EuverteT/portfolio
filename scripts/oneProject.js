...
for (let project of projects) {
    if (urlProjectName == project.name) {
        ...
        if (project.galeryImages) {
            ...
            //Création d'une modale pour les projets Piiquante V1 et Groupomania V1
            if ((urlProjectName === "Piiquante v1") || (urlProjectName === "Groupomania v1")) {

                //bouton
                let modalBtn = document.createElement("a")
                modalBtn.setAttribute("class", "openOrDownloadLink")
                modalBtn.style.cursor = "pointer"
                openOrDownloadBlock.appendChild(modalBtn)
                modalBtn.innerText = displayDownloadButton

                //modale
                let modal = document.createElement("div")
                modal.setAttribute("class", "modal")
                modal.style.display = "none"
                openOrDownloadBlock.appendChild(modal)
                ...
                //modal lineone
                let modalLineOne = document.createElement("div")
                modalLineOne.setAttribute("class", "modalLineOne")
                modalLineOne.innerText = displayWarningP6P7
                modalContent.appendChild(modalLineOne)

                let spann = document.createElement("span")
                spann.setAttribute("class", "close")
                spann.innerText = "\xD7"
                modalLineOne.appendChild(spann)

                //modal linetwo
                let modalLineTwo = document.createElement("div")
                modalLineTwo.setAttribute("class", "modalLineTwo")
                modalContent.appendChild(modalLineTwo)

                let yesLink = document.createElement("a") // container a incluant le bouton Yes
                yesLink.setAttribute("href", project.downloadProjectLink)
                yesLink.setAttribute("type", "download")
                modalLineTwo.appendChild(yesLink)

                let yesAnswer = document.createElement("button")
                yesAnswer.setAttribute("id", "yesAnswer")
                yesAnswer.innerText = yesResponse
                yesLink.appendChild(yesAnswer)

                let noAnswer = document.createElement("button")
                noAnswer.setAttribute("id", "noAnswer")
                noAnswer.innerText = noResponse
                modalLineTwo.appendChild(noAnswer)
                ...
                //fermeture par un clic à l'extérieur
                window.onclick = function (event) {
                    if (event.target == modal) {
                        modal.style.display = "none";
                    }
                }
                // L'écoute n'est pas nécessaire grace au container a incluant le lien de téléchargement
                // yesAnswer.addEventListener('click', function () {      
                // })
                noAnswer.addEventListener('click', function (event) {
            ...
            if ((urlProjectName !== "Piiquante v1") && (urlProjectName !== "Groupomania v1")) {
                ...
                let downloadProjectLink = document.createElement("a")
                downloadProjectLink.setAttribute("href", project.downloadProjectLink)
                downloadProjectLink.setAttribute("type", "download")
                downloadProjectLink.setAttribute("class", "openOrDownloadLink")
                downloadProjectLink.innerText = displayDownloadButton
                openOrDownloadBlock.appendChild(downloadProjectLink)
            }
